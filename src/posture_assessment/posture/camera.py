...
class MockCameraAdapter(DepthCameraAdapter):
    ...
    def probe(self) -> DeviceInfo:
        return DeviceInfo(
            ...
            model="Azure Kinect DK Simulator",
            tracking_mode="CPU (simulation)",
            message=(
                "No hardware connected; using deterministic simulated RGB-D data"
                + (f"; fallback reason: {self.fallback_message}" if self.fallback_message else "")
            ),
            is_mock=True,
