...
class UserImportService:
    ...
    def create_template(self, destination: Path) -> Path:
        ...
        sheet.append(list(ENGLISH_FIELDS.values()))
        sheet.append(
            [
                "",
                "Test User",
    ...
    def _write_error_log(
        ...
        with path.open("w", encoding="utf-8-sig", newline="") as handle:
            writer = csv.DictWriter(
                handle,
                fieldnames=["Row", "Patient ID", "Name", "Error Field", "Original Value", "Action"],
            )
            writer.writeheader()
            for item in errors:
                writer.writerow(
...
class UserExportService:
    ...
    def export(self, options: ExportOptions, destination: Path | None = None) -> ExportResult:
        ...
        with tempfile.TemporaryDirectory(dir=destination_dir) as staging_text:
            ...
            exported_ids = {
                user.id: user_rows[index]["Patient ID"] for index, user in enumerate(users)
            }
            session_rows = [
    ...
    def _user_row(user: User, anonymize: bool) -> dict[str, Any]:
        ...
        if anonymize:
            ...
            email = ""
        return {
            "Patient ID": patient_no,
            "Name": name,
            "Birth Date": user.birth_date.isoformat(),
            "Sex": display_sex(user.sex),
            "Mobile": mobile,
            "Height (cm)": user.height_cm,
            "Weight (kg)": user.weight_kg,
            "Address": address,
            "Email": email,
            "Profile Complete": "Yes" if user.profile_complete else "No",
        }
    ...
    def _session_row(
        ...
        return {
            ...
            "Assessment Module": display_module(item.assessment_type),
            "Status": display_status(item.status),
            "Assessment Time": item.started_at.isoformat(timespec="seconds"),
            "Report Path": item.report_path or "",
        }
