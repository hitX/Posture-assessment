...
from posture_assessment.localization import display_direction, display_metric
...
def pelvis_overlay_pixmap(
    ...
    painter.drawText(18, 30, f"{view.label} · Hip Center / Pelvis Model Proxy Points")
    painter.drawText(18, height - 18, "Red: hip axis   Green: support base   Yellow: support centerline")
    painter.end()
    return canvas
...
def pelvis_capture_pixmap(
    ...
    for index, (label, pixmap) in enumerate(
        (("RGB Body Outline", rgb), ("Depth Pseudocolor + Skeleton", depth), ("Hip ROI + Proxy Points", overlay))
    ):
    ...
    painter.drawText(
        12,
        height - 10,
        f"Distance {pelvis[2] / 1000.0:.2f} m   Depth coverage {coverage:.1%}   Medium+ joints {confidence}/32",
    )
    painter.end()
    return canvas
...
class PelvisCaptureDialog(CaptureDialog):
    def _preview_ready(self, frame: FrameBundle) -> None:
        ...
        self.status.setText("● RGB / depth / hip proxy preview ready; the stability quality gate runs after capture")
...
class PelvisPage(QWidget):
    ...
    def _build_overview(self) -> QWidget:
        ...
        title = QLabel("Single Azure Kinect Pelvis Posture Screening")
        title.setObjectName("pageTitle")
        title_box.addWidget(title)
        self.device_label = QLabel("Run device, ground calibration, and Body Tracking checks first")
        ...
        note = QLabel(
            "Capture order: Front → Left Side → Back → Right Side. Wear fitted clothing with an unobstructed pelvis outline; "
            "this module reports posture screening proxy values and does not reconstruct ASIS/PSIS or bones."
        )
        note.setWordWrap(True)
        note.setObjectName("muted")
        root.addWidget(note)
        footer = QHBoxLayout()
        self.summary_label = QLabel("Views: 0 / 4   Depth coverage: -   Joint confidence: -")
        self.summary_label.setObjectName("muted")
        footer.addWidget(self.summary_label)
        footer.addStretch()
        self.analyze_button = QPushButton("Generate Pelvis Screening Result")
    ...
    def _build_analysis(self) -> QWidget:
        ...
        title = QLabel("Pelvis Posture Proxy Metric Calculation")
        title.setObjectName("pageTitle")
        root.addWidget(title, alignment=Qt.AlignmentFlag.AlignCenter)
        self.analysis_status = QLabel("Aligning gravity, checking cross-view consistency, and fusing confidence")
    ...
    def _build_result(self) -> QWidget:
        ...
        left = QVBoxLayout()
        title = QLabel("Pelvis Screening Result · 2D Proxy Overlay")
        title.setObjectName("pageTitle")
        left.addWidget(title)
        self.result_visual = QLabel("Waiting for result")
        ...
        right.addWidget(QLabel("Structured Experimental Metrics"))
        self.result_table = QTableWidget(0, 4)
        self.result_table.setHorizontalHeaderLabels(["Metric", "Value", "Source", "Confidence"])
        ...
        disclaimer = QLabel(
            "Disclaimer: all metrics are posture-screening proxy values and experimental results; the SDK pelvis pose is not "
            "a clinical ASIS–PSIS pelvic angle and must not be used for diagnosis or medical grading without local validation."
        )
        disclaimer.setWordWrap(True)
        disclaimer.setStyleSheet(f"color:{PINK}; font-weight:700;")
        right.addWidget(disclaimer)
        buttons = QHBoxLayout()
        back = QPushButton("Return to Capture / Recapture")
        back.clicked.connect(lambda: self.stack.setCurrentWidget(self.overview_page))
        buttons.addWidget(back)
        self.report_button = QPushButton("Submit Structured Result")
    ...
    def set_detection_context(self, user: User, assessment: AssessmentSession) -> None:
        ...
        self.patient_bar.setText(
            f"Subject: {user.patient_no} / {user.name}   Module: Pelvis Posture Screening   "
        ...
        self.analyze_button.setEnabled(False)
        self.summary_label.setText("Views: 0 / 4   Depth coverage: -   Joint confidence: -")
    ...
    def _device_check(self) -> None:
        ...
        self.device_label.setText("Checking camera, Body Tracking, and ground calibration…")
    ...
    def _device_ready(self, info: DeviceInfo) -> None:
        ...
        mode = info.tracking_mode or "Not reported"
        self.device_label.setText(
            f"● Connected: {info.model} / {info.serial_number}   Processing mode: {mode}"
        )
    ...
    def _device_failed(self, message: str) -> None:
        self.self_check_button.setEnabled(True)
        self.device_label.setText("Device or environment check failed")
        show_error(self, "Pelvis assessment check failed", message)

    def _open_capture(self, view: PoseKind) -> None:
        if self.assessment is None:
            show_info(self, "No subject selected", "Start an assessment from User Profiles first.")
            return
        self.state = PelvisUiState.CAPTURE
        dialog = PelvisCaptureDialog(self.service, self.assessment.id, view, self)
        dialog.setWindowTitle(f"Pelvis Assessment - {view.label}")
        dialog.instruction.setText(
            f"Stand naturally for {view.label} with the pelvis unobstructed; save a 2-second RGB-D window when stable."
        )
    ...
    def _update_summary(self) -> None:
        ...
        self.summary_label.setText(
            f"Views: {len(passed)} / 4   Average depth coverage: {depth:.1%}   Valid joints: {joints:.0f}/32"
        )
    ...
    def _analysis_ready(self, result: PelvisAnalysisResult) -> None:
        ...
        if result.retake_views:
            ...
            show_info(self, "Recapture required", f"The following views failed the quality gate: {labels}")
            self.stack.setCurrentWidget(self.overview_page)
            return
    ...
    def _analysis_failed(self, message: str) -> None:
        self.state = PelvisUiState.OVERVIEW
        self.stack.setCurrentWidget(self.overview_page)
        show_error(self, "Pelvis screening analysis failed", message)
    ...
    def _populate_result(self, result: PelvisAnalysisResult) -> None:
        ...
        for row, item in enumerate(rows):
            ...
            direction = display_direction(item.direction)
            value = f"{direction + ' ' if direction else ''}{item.value:g}{item.unit}"
            for column, text in enumerate((display_metric(item.name), value, source, f"{item.confidence:.1f}%")):
                self.result_table.setItem(row, column, QTableWidgetItem(text))
        if result.review_reasons:
            self.review_label.setText("Manual review / recapture recommendations:\n" + "\n".join(result.review_reasons))
            self.report_button.setText("Submit Pending-Review Result")
        else:
            self.review_label.setText(
                f"Algorithm version: {result.algorithm_version}   Cross-view consistency check passed"
            )
            self.report_button.setText("Submit Structured Result")
    ...
    def _submit_result(self) -> None:
        ...
        if self.result.review_reasons:
            response = QMessageBox.question(
                self,
                "Submit Pending-Review Result",
                "The result contains cross-view differences and will be submitted as pending review. Continue?",
            )
            if response != QMessageBox.StandardButton.Yes:
                return
        self.analysis_ready.emit(self.assessment.id)
        show_info(self, "Structured Result Ready", "The report module can read pelvis metrics, quality data, the algorithm version, and the disclaimer.")
